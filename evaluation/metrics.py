...
def evaluator(gt_paths, pred_paths, metrics=['S', 'MAE', 'E', 'F', 'WF', 'HCE'], verbose=False):
    ...
    for idx_sample in tqdm(range(len(gt_paths)), total=len(gt_paths)) if verbose else range(len(gt_paths)):
        ...
        if pred_ary is None or gt_ary is None:
            print(f"跳过无效图像: {os.path.basename(pred)}")  # ✅ 正确：pred是字符串
            continue

        if pred_ary.size == 0 or gt_ary.size == 0:
            print(f"跳过空图像: {os.path.basename(pred)}")  # 建议也加上文件名
            continue
            #####
...
#     return pred, gt
#######--------------------------------------------------------
####lsn____________________________________________________________
def _prepare_data(pred: np.ndarray, gt: np.ndarray) -> tuple:
    """
    Robust prepare:
    - Ensure pred is float in [0,1], normalized per-image.
    - Ensure gt is binary boolean mask (True for foreground).
      Use adaptive threshold:
        * if gt dtype is uint8 and gt.max() >= 128 -> use gt>128 (common case)
        * elif gt.max() > 1 -> use gt > (gt.max()/2)
        * else (float in [0,1]) use gt>0.5
    """
    # Make sure we operate on copy and numeric types
    pred = pred.astype(np.float32)
    gt = gt.astype(np.float32)

    # Normalize prediction to [0,1]
    # If pred is already in [0,1] (max <=1), keep it; else assume 0-255
    if pred.max() > 1.0:
        pred = pred / 255.0

    # If pred is constant, leave it (downstream code handles min==max)
    if pred.max() != pred.min():
        pred = (pred - pred.min()) / (pred.max() - pred.min())

    # Prepare GT as boolean mask
    gmax = gt.max()
    if gmax > 1.0:
        # GT currently in 0..255 or 0..N
        if gmax >= 128.0:
            # common standard: foreground encoded as 255
            gt_bin = gt > 128
        else:
            # unusual: foreground encoded with some value <128 (e.g. 76)
            # use adaptive threshold at half the max value
            gt_bin = gt > (gmax / 2.0)
    else:
        # GT already normalized to [0,1] floats
        gt_bin = gt > 0.5
