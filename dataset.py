...
class MyData(data.Dataset):
    def __init__(self, datasets, image_size, is_train=True):
        ...
        # dataset_root = data_root_dir + task = /home/user/lc/datasets/COD
    ...
    def __getitem__(self, index):
        ...
        # label = self.transform_label(label)    #.squeeze(0)  # 转换label并去掉多余的维度
        # ****************
...
if __name__ == '__main__':
    ...
    dataset = MyData(datasets=config.training_set, image_size=config.size, is_train=True)
    print("数据集路径:", dataset.image_paths)
